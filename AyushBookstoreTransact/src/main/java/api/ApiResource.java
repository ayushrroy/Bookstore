...
public class ApiResource {
    ...
    //implemented by ayush to fetch category id for category name
    @GET
    ...
    public List<Book> booksByCategoryName(@PathParam("category-name") String categoryName,
                                        @Context HttpServletRequest httpRequest) {
        try {
                long categoryId = categoryDao.findCategoryIdByName(categoryName);
                Category category = categoryDao.findByCategoryId(categoryId);
                if (category == null) {
    ...
    public OrderDetails placeOrder(OrderForm orderForm) {
        ...
        try {
            ...
            System.out.println(orderForm.getCart().getItems());//added by ayush
            if (orderId > 0) {
